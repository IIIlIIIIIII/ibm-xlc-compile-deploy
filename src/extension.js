...
function activate(context) {
    ...
    ensureConfigFile();
    context.subscriptions.push(vscode.commands.registerCommand('ibm-xlc-compile-deploy.configureMappings', async () => {
        ensureConfigFile();
        ...
        if (localConfig.src && ftpConfig.remoteSrc && relativePath.startsWith(localConfig.src)) {
            await (0, ftp_1.uploadFile)(relativePath, localConfig.src, ftpConfig.remoteSrc, ftpConfig, workspaceFolder);
            ...
            await (0, ftp_1.uploadFile)(relativePath, localConfig.mak, ftpConfig.remoteMak, ftpConfig, workspaceFolder);
            ...
            await (0, ftp_1.uploadFile)(relativePath, localConfig.inc, ftpConfig.remoteInc, ftpConfig, workspaceFolder);
        ...
        if (localConfig.inc && ftpConfig.remoteInc) {
            const confirm = await vscode.window.showWarningMessage('是否跳过上传 inc 文件夹（会覆盖远程内容）？', { modal: true }, '确认');
            if (confirm !== '确认') {
                await (0, ftp_1.uploadFolder)(localConfig.inc, ftpConfig.remoteInc, ftpConfig, workspaceFolder);
        ...
        const confirm = await vscode.window.showWarningMessage('是否clean？', { modal: true }, '确认');
        cleanCommand = compileCommand + " clean";
        if (confirm === '确认') {
            await (0, ssh_1.runRemoteCommand)(config.ftpConfig, cleanCommand, '清理');
        }
        await (0, ssh_1.runRemoteCommand)(config.ftpConfig, compileCommand, '编译');
        ...
        await (0, ssh_1.runRemoteCommand)(config.ftpConfig, deployCommand, '发布');
    }));
}
